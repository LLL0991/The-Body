...
/** 匹配顺序：更具体的在前（练后+晚/午/摄入 > 练后 > 练前 > 早/午/晚） */
const MEAL_PATTERNS = [
  { pattern: /练后摄入|练后晚餐|练后.*[晚夜]|练后.*主餐/, nameContains: ['练后', '摄入'] },
  { pattern: /练后.*[午]|[午].*练后|练后[午饭]|练后午餐/, nameContains: ['练后', '午'] },
  { pattern: /练后即刻|练后立刻|练完后?|训练后|撸铁后|练后(?!午|晚|入)/, nameContains: ['练后'] },
  { pattern: /练前补充|练前/, nameContains: ['练前'] },
